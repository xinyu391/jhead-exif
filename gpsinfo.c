...
//--------------------------------------------------------------------------
void ProcessGpsInfo(unsigned char * DirStart, unsigned char * OffsetBase, unsigned ExifLength, ImageInfo_t* exifInfo)
{
    int de;
    unsigned a;
    int NumDirEntries;
    ...
    exifInfo->GpsInfoPresent = TRUE;
    strcpy(exifInfo->GpsLat, "? ?");
    strcpy(exifInfo->GpsLong, "? ?");
    exifInfo->GpsAlt[0] = 0; 
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        switch(Tag){
            ...
            case TAG_GPS_LAT_REF:
                exifInfo->GpsLat[0] = ValuePtr[0];
                break;

            case TAG_GPS_LONG_REF:
                exifInfo->GpsLong[0] = ValuePtr[0];
                break;
            ...
            case TAG_GPS_LONG:
                ...
                for (a=0;a<3;a++){
                    ...
                    den = Get32s(ValuePtr+4+a*ComponentSize, exifInfo->ptr.MotorolaOrder);
                    digits = 0;
                    ...
                    Values[a] = ConvertAnyFormat(ValuePtr+a*ComponentSize, Format, exifInfo->ptr.MotorolaOrder);
                }

                sprintf(TempString, FmtString, Values[0], Values[1], Values[2]);

                if (Tag == TAG_GPS_LAT){
                    strncpy(exifInfo->GpsLat+2, TempString, 29);
                }else{
                    strncpy(exifInfo->GpsLong+2, TempString, 29);
                }
                break;

            case TAG_GPS_ALT_REF:
                exifInfo->GpsAlt[0] = (char)(ValuePtr[0] ? '-' : ' ');
                break;

            case TAG_GPS_ALT:
                sprintf(exifInfo->GpsAlt + 1, "%.2fm", 
                    ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder));
                break;
        }
        ...
        if (ShowTags){
            ...
            switch(Format){
                ...
                default:
                    ...
                    for (a=0;;){
                        PrintFormatNumber(ValuePtr+a*ComponentSize, Format, ByteCount, exifInfo->ptr.MotorolaOrder);
