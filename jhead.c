...
static int ModifyDescriptComment(char * OutComment, char * SrcComment)
{
    ...
    if (!HasScandate && !exifInfo->DateTime[0]){
        // Scan date is not in the file yet, and it doesn't have one built in.  Add it.
        char Temp[40];
        sprintf(Temp, "scan_date=%s", ctime(&exifInfo->FileDateTime));
...
static int AutoResizeCmdStuff(void)
{
    ...
    scale = TargetSize / exifInfo->Width;
    if (scale > TargetSize / exifInfo->Height) scale = TargetSize / exifInfo->Height;

    if (scale > 0.8){
        if (exifInfo->QualityGuess >= 93){
            // Re-compress at lower quality.
            sprintf(CommandString, "mogrify -quality 86 &i");
            return TRUE;
        }
        printf("not resizing %dx%x '%s'\n",exifInfo->Height, exifInfo->Width, exifInfo->FileName);
        return FALSE;
    }

    if (scale < 0.4) scale = 0.4; // Don't scale down by too much.

    sprintf(CommandString, "mogrify -geometry %dx%d -quality 85 &i",(int)(exifInfo->Width*scale+0.5), 
                                    (int)(exifInfo->Height*scale+0.5));
    return TRUE;
}
...
//--------------------------------------------------------------------------
static int CheckFileSkip(ImageInfo_t* exifInfo)
{
    ...
    if (FilterModel){
        ...
        if (strstr(exifInfo->CameraModel, FilterModel) == NULL){
            // Skip.
            return TRUE;
        }
    }
    ...
    if (ExifOnly){
        ...
        if (FindSection(M_EXIF, exifInfo) == NULL){
            return TRUE;
        }
    }

    if (PortraitOnly == 1){
        if (exifInfo->Width > exifInfo->Height) return TRUE;
    }
...
//--------------------------------------------------------------------------
static void DoFileRenaming(const char * FileName, ImageInfo_t* exifInfo)
{
    ...
    if (strftime_args){
        ...
        UnixTime = mktime(&tm);
        if ((int)UnixTime == -1){
            printf("Could not convert %s to unix time",exifInfo->DateTime);
            return;
        }
...
//--------------------------------------------------------------------------
static int DoAutoRotate(const char * FileName, ImageInfo_t* exifInfo)
{
    if (exifInfo->Orientation != 1){
        const char * Argument;
        Argument = ClearOrientation(exifInfo);
        ...
        if (!ZeroRotateTagOnly){
            ...
            if (exifInfo->ThumbnailOffset && 
                exifInfo->ThumbnailSize && 
                exifInfo->ThumbnailAtEnd){
...
//--------------------------------------------------------------------------
static int RegenerateThumbnail(const char * FileName, ImageInfo_t* exifInfo)
{
    ...
    if (system(ThumbnailGenCommand) == 0){
        // Put the thumbnail back in the header
        return ReplaceThumbnail(FileName, exifInfo);
    }else{
...
{
    struct tm ts;
    ts = *localtime(&exifInfo->FileDateTime);
...
static void ProcessFile(const char * FileName)
{
    ...
    if (TrimExifTrailingZeroes){
        if (exifInfo->ThumbnailAtEnd){
            Section_t * ExifSection;
            int NumRedundant, WasRedundant;
            unsigned char * StartRedundant;
            //printf("Exif: Thumbnail %d - %d\n",exifInfo->ThumbnailOffset, exifInfo->ThumbnailOffset+exifInfo->ThumbnailSize);
            ExifSection = FindSection(M_EXIF);

            StartRedundant = ExifSection->Data + 8 + exifInfo->ThumbnailOffset+exifInfo->ThumbnailSize;
            WasRedundant = NumRedundant = (ExifSection->Size) - (exifInfo->ThumbnailOffset + exifInfo->ThumbnailSize + 8);
            ...
            if (NumRedundant != WasRedundant){
                ...
                NewExifSize = exifInfo->ThumbnailOffset + exifInfo->ThumbnailSize + 8 + NumRedundant;
    ...
    if (ExifTimeAdjust || ExifTimeSet || DateSetChars || FileTimeToExif){
       if (exifInfo->numDateTimeTags){
            ...
            if (ExifTimeSet){
                ...
                if (FileTimeToExif){
                    FileTimeAsString(exifInfo->DateTime);
                }
                if (DateSetChars){
                    memcpy(exifInfo->DateTime, DateSet, DateSetChars);
                    a = 1970;
                    sscanf(DateSet, "%d", &a);
                    if (a < 1970){
                        strcpy(TempBuf, exifInfo->DateTime);
                        goto skip_unixtime;
                    }
                }
                // A time offset to adjust by was specified.
                if (!Exif2tm(&tm, exifInfo->DateTime)) goto badtime;
...
skip_unixtime:         
            ...
            for (a = 0; a < exifInfo->numDateTimeTags; a++) {
                uchar * Pointer;
                Pointer = ExifSection->Data+exifInfo->DateTimeOffsets[a]+8;
                memcpy(Pointer, TempBuf, 19);
            }
            memcpy(exifInfo->DateTime, TempBuf, 19);
