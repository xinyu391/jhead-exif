...
static void ProcessCanonMakerNoteDir(unsigned char * DirStart, unsigned char * OffsetBase, 
        unsigned ExifLength, ImageInfo_t* exifInfo)
{
    int de;
    int a;
    int NumDirEntries;
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if (Tag == 1 && Components > 16){
            int IsoCode = Get16u(ValuePtr + 16*sizeof(unsigned short), exifInfo->ptr.MotorolaOrder);
            if (IsoCode >= 16 && IsoCode <= 24){
                exifInfo->ISOequivalent = 50 << (IsoCode-16);
            } 
        }

        if (Tag == 4 && Format == FMT_USHORT){
            if (Components > 7){
                int WhiteBalance = Get16u(ValuePtr + 7*sizeof(unsigned short), exifInfo->ptr.MotorolaOrder);
                switch(WhiteBalance){
                    // 0=Auto, 6=Custom
                    case 1: exifInfo->LightSource = 1; break; // Sunny
                    case 2: exifInfo->LightSource = 1; break; // Cloudy
                    case 3: exifInfo->LightSource = 3; break; // Thungsten
                    case 4: exifInfo->LightSource = 2; break; // Fourescent
                    case 5: exifInfo->LightSource = 4; break; // Flash
                }
            }
            if (Components > 19 && exifInfo->Distance <= 0) {
                // Indicates the distance the autofocus camera is focused to.
                // Tends to be less accurate as distance increases.
                int temp_dist = Get16u(ValuePtr + 19*sizeof(unsigned short), exifInfo->ptr.MotorolaOrder);
                if (temp_dist != 65535){
                    exifInfo->Distance = (float)temp_dist/100;
                }else{
                    exifInfo->Distance = -1 /* infinity */;
                }
            }
        }
    }
...
{
    if (strstr(exifInfo->CameraMake, "Canon")){
        ...
        MotorolaOrderSave = exifInfo->ptr.MotorolaOrder;
        exifInfo->ptr.MotorolaOrder = 0; // Temporarily switch to little endian.
        ProcessCanonMakerNoteDir(ValuePtr, OffsetBase, ExifLength, exifInfo);
        exifInfo->ptr.MotorolaOrder = MotorolaOrderSave;
    }else{
        if (ShowTags){
