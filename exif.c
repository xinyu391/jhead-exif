...
// static void * exifInfo->ptr.OrientationPtr[2];
// static int    exifInfo->ptr.OrientationNumFormat[2];
// int exifInfo->ptr.NumOrientations = 0;
...
//--------------------------------------------------------------------------
static void Put16u(void * Short, unsigned short PutValue, int MotorolaOrder)
{
    if (MotorolaOrder){
...
void PrintFormatNumber(void * ValuePtr, int Format, int ByteCount, int MotorolaOrder)
{
    ...
    for(n=0;n<16;n++){
        switch(Format){
            ...
            case FMT_USHORT:    printf("%d",Get16u(ValuePtr, MotorolaOrder)); s=2;      break;
            case FMT_ULONG:     
            case FMT_SLONG:     printf("%d",Get32s(ValuePtr, MotorolaOrder)); s=4;      break;
            case FMT_SSHORT:    printf("%hd",(signed short)Get16u(ValuePtr, MotorolaOrder)); s=2; break;
            case FMT_URATIONAL:
                printf("%u/%u",Get32s(ValuePtr, MotorolaOrder), Get32s(4+(char *)ValuePtr, MotorolaOrder)); 
                s = 8;
                break;

            case FMT_SRATIONAL: 
                printf("%d/%d",Get32s(ValuePtr, MotorolaOrder), Get32s(4+(char *)ValuePtr, MotorolaOrder)); 
                s = 8;
                break;
...
static void ProcessExifDir(unsigned char * DirStart, unsigned char * OffsetBase, 
        unsigned ExifLength, int NestingLevel, ImageInfo_t* exifInfo)
{
    int de;
    int a;
    int NumDirEntries;
    ...
    NumDirEntries = Get16u(DirStart, exifInfo->ptr.MotorolaOrder);
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        Tag = Get16u(DirEntry, exifInfo->ptr.MotorolaOrder);
        Format = Get16u(DirEntry+2, exifInfo->ptr.MotorolaOrder);
        Components = Get32u(DirEntry+4, exifInfo->ptr.MotorolaOrder);
        ...
        if (ByteCount > 4){
            unsigned OffsetVal;
            OffsetVal = Get32u(DirEntry+8, exifInfo->ptr.MotorolaOrder);
            ...
            if (OffsetVal > exifInfo->LargestExifOffset){
                exifInfo->LargestExifOffset = OffsetVal;
            }
        ...
        if (Tag == TAG_MAKER_NOTE){
            ...
            ProcessMakerNote(ValuePtr, ByteCount, OffsetBase, ExifLength, exifInfo);
            continue;
        }
            ...
            switch(Format){
                case FMT_BYTE:
                    if(ByteCount>1){
                        ...
                        PrintFormatNumber(ValuePtr, Format, ByteCount, exifInfo->ptr.MotorolaOrder);
                        printf("\n");
                    }
                    break;
        ...
        switch(Tag){
            ...
            case TAG_MAKE:
                strncpy(exifInfo->CameraMake, (char *)ValuePtr, ByteCount < 31 ? ByteCount : 31);
                break;

            case TAG_MODEL:
                strncpy(exifInfo->CameraModel, (char *)ValuePtr, ByteCount < 39 ? ByteCount : 39);
                break;

            case TAG_DATETIME_ORIGINAL:
                // If we get a DATETIME_ORIGINAL, we use that one.
                strncpy(exifInfo->DateTime, (char *)ValuePtr, 19);
                // Fallthru...

            case TAG_DATETIME_DIGITIZED:
            case TAG_DATETIME:
                if (!isdigit(exifInfo->DateTime[0])){
                    // If we don't already have a DATETIME_ORIGINAL, use whatever
                    // time fields we may have.
                    strncpy(exifInfo->DateTime, (char *)ValuePtr, 19);
                }

                if (exifInfo->numDateTimeTags >= MAX_DATE_COPIES){
                    ErrNonfatal("More than %d date fields in Exif.  This is nuts", MAX_DATE_COPIES, 0);
                    break;
                }
                exifInfo->DateTimeOffsets[exifInfo->numDateTimeTags++] = 
                    (char *)ValuePtr - (char *)OffsetBase;
                break;

            case TAG_WINXP_COMMENT:
                if (exifInfo->Comments[0]){ // We already have a jpeg comment.
                ...
                if (ByteCount > 1){
                    ...
                    memcpy(exifInfo->Comments, ValuePtr, ByteCount);
                    exifInfo->CommentWidthchars = ByteCount/2;
                }
                break;
                ...
                // Copy the comment
                {
                    ...
                    if (msiz > 5 && memcmp(ValuePtr, "ASCII",5) == 0){
                        for (a=5;a<10 && a < msiz;a++){
                            int c = (ValuePtr)[a];
                            if (c != '\0' && c != ' '){
                                strncpy(exifInfo->Comments, (char *)ValuePtr+a, msiz-a);
                                break;
                            }
                        }
                    }else{
                        strncpy(exifInfo->Comments, (char *)ValuePtr, msiz);
                    }
                }
                break;
            ...
            case TAG_FNUMBER:
                ...
                exifInfo->ApertureFNumber = (float)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;
            ...
            case TAG_MAXAPERTURE:
                ...
                if (exifInfo->ApertureFNumber == 0){
                    exifInfo->ApertureFNumber 
                        = (float)exp(ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder)*log(2)*0.5);
                }
                break;
            ...
            case TAG_FOCALLENGTH:
                ...
                exifInfo->FocalLength = (float)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_SUBJECT_DISTANCE:
                // Inidcates the distacne the autofocus camera is focused to.
                // Tends to be less accurate as distance increases.
                exifInfo->Distance = (float)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_EXPOSURETIME:
                // Simplest way of expressing exposure time, so I trust it most.
                // (overwrite previously computd value if there is one)
                exifInfo->ExposureTime = (float)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_SHUTTERSPEED:
                // More complicated way of expressing exposure time, so only use
                // this value if we don't already have it from somewhere else.
                if (exifInfo->ExposureTime == 0){
                    exifInfo->ExposureTime 
                        = (float)(1/exp(ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder)*log(2)));
                }
                break;


            case TAG_FLASH:
                exifInfo->FlashUsed=(int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_ORIENTATION:
                if (exifInfo->ptr.NumOrientations >= 2){
                ...
                exifInfo->ptr.OrientationPtr[exifInfo->ptr.NumOrientations] = ValuePtr;
                exifInfo->ptr.OrientationNumFormat[exifInfo->ptr.NumOrientations] = Format;
                if (exifInfo->ptr.NumOrientations == 0){
                    exifInfo->Orientation = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                }
                if (exifInfo->Orientation < 0 || exifInfo->Orientation > 8){
                    ErrNonfatal("Undefined rotation value %d in Exif", exifInfo->Orientation, 0);
                }
                exifInfo->ptr.NumOrientations += 1;
                break;
            ...
            case TAG_PIXEL_X_DIMENSION:
                ...
                a = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                if (exifInfo->ptr.ExifImageWidth < a) exifInfo->ptr.ExifImageWidth = a;
                break;

            case TAG_FOCAL_PLANE_XRES:
                exifInfo->ptr.FocalplaneXRes = ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_FOCAL_PLANE_UNITS:
                switch((int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder)){
                    case 1: exifInfo->ptr.FocalplaneUnits = 25.4; break; // inch
                    case 2: 
                        // According to the information I was using, 2 means meters.
                        // But looking at the Cannon powershot's files, inches is the only
                        // sensible value.
                        exifInfo->ptr.FocalplaneUnits = 25.4;
                        break;

                    case 3: exifInfo->ptr.FocalplaneUnits = 10;   break;  // centimeter
                    case 4: exifInfo->ptr.FocalplaneUnits = 1;    break;  // millimeter
                    case 5: exifInfo->ptr.FocalplaneUnits = .001; break;  // micrometer
                }
                break;

            case TAG_EXPOSURE_BIAS:
                exifInfo->ExposureBias = (float)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_WHITEBALANCE:
                exifInfo->Whitebalance = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_LIGHT_SOURCE:
                exifInfo->LightSource = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_METERING_MODE:
                exifInfo->MeteringMode = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_EXPOSURE_PROGRAM:
                exifInfo->ExposureProgram = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_EXPOSURE_INDEX:
                if (exifInfo->ISOequivalent == 0){
                    ...
                    exifInfo->ISOequivalent = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                }
                break;

            case TAG_EXPOSURE_MODE:
                exifInfo->ExposureMode = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;
            ...
            case TAG_DIGITALZOOMRATIO:
                exifInfo->DigitalZoomRatio = (float)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_THUMBNAIL_OFFSET:
                ThumbnailOffset = (unsigned)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                exifInfo->ptr.DirWithThumbnailPtrs = DirStart;
                break;

            case TAG_THUMBNAIL_LENGTH:
                ThumbnailSize = (unsigned)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                exifInfo->ThumbnailSizeOffset = ValuePtr-OffsetBase;
                break;
            ...
            case TAG_INTEROP_OFFSET:
                if (Tag == TAG_INTEROP_OFFSET && ShowTags) printf("%s    Interop Dir:",IndentString);
                {
                    ...
                    SubdirStart = OffsetBase + Get32u(ValuePtr, exifInfo->ptr.MotorolaOrder);
                    if (SubdirStart < OffsetBase || SubdirStart > OffsetBase+ExifLength){
                        ErrNonfatal("Illegal Exif or interop ofset directory link",0,0);
                    }else{
                        ProcessExifDir(SubdirStart, OffsetBase, ExifLength, NestingLevel+1, exifInfo);
                    }
                    continue;
                }
                break;
            ...
            case TAG_GPSINFO:
                if (ShowTags) printf("%s    GPS info dir:",IndentString);
                {
                    ...
                    if (SubdirStart < OffsetBase || SubdirStart > OffsetBase+ExifLength){
                        ...
                        ProcessGpsInfo(SubdirStart, OffsetBase, ExifLength, exifInfo);
                    }
                    continue;
                }
                break;
            ...
            case TAG_FOCALLENGTH_35MM:
                ...
                exifInfo->FocalLength35mmEquiv = (unsigned)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;

            case TAG_DISTANCE_RANGE:
                // Three possible standard values:
                //   1 = macro, 2 = close, 3 = distant
                exifInfo->DistanceRange = (int)ConvertAnyFormat(ValuePtr, Format, exifInfo->ptr.MotorolaOrder);
                break;
            ...
            case TAG_X_RESOLUTION:
                if (NestingLevel==0) {// Only use the values from the top level directory
                    exifInfo->xResolution = (float)ConvertAnyFormat(ValuePtr,Format, exifInfo->ptr.MotorolaOrder);
                    // if yResolution has not been set, use the value of xResolution
                    if (exifInfo->yResolution == 0.0) exifInfo->yResolution = exifInfo->xResolution;
                }
                break;

            case TAG_Y_RESOLUTION:
                if (NestingLevel==0) {// Only use the values from the top level directory
                    exifInfo->yResolution = (float)ConvertAnyFormat(ValuePtr,Format, exifInfo->ptr.MotorolaOrder);
                    // if xResolution has not been set, use the value of yResolution
                    if (exifInfo->xResolution == 0.0) exifInfo->xResolution = exifInfo->yResolution;
                }
                break;

            case TAG_RESOLUTION_UNIT:
                if (NestingLevel==0) {// Only use the values from the top level directory
                    exifInfo->ResolutionUnit = (int) ConvertAnyFormat(ValuePtr,Format, exifInfo->ptr.MotorolaOrder);
                }
                break;
...

    {
        ...
        if (DIR_ENTRY_ADDR(DirStart, NumDirEntries) + 4 <= OffsetBase+ExifLength){
            Offset = Get32u(DirStart+2+12*NumDirEntries, exifInfo->ptr.MotorolaOrder);
            if (Offset){
                ...
                if (Offset > exifInfo->LargestExifOffset){
                    exifInfo->LargestExifOffset = Offset;
                }
            }
        }else{
    ...
    if (ThumbnailOffset){
        ...
        if (ThumbnailOffset <= ExifLength){
            ...
            exifInfo->ThumbnailOffset = ThumbnailOffset;
            exifInfo->ThumbnailSize = ThumbnailSize;
...
//--------------------------------------------------------------------------
void process_EXIF (unsigned char * ExifSection, unsigned int length, ImageInfo_t* exifInfo)
{
    unsigned int FirstOffset;

    exifInfo->ptr.FocalplaneXRes = 0;
    exifInfo->ptr.FocalplaneUnits = 0;
    exifInfo->ptr.ExifImageWidth = 0;
    exifInfo->ptr.NumOrientations = 0;
    ...
    if (Get16u(ExifSection+10, exifInfo->ptr.MotorolaOrder) != 0x2a){
        ErrNonfatal("Invalid Exif start (1)",0,0);
        return;
    }

    FirstOffset = Get32u(ExifSection+12, exifInfo->ptr.MotorolaOrder);
    ...
    exifInfo->ptr.DirWithThumbnailPtrs = NULL;


    // First directory starts 16 bytes in.  All offset are relative to 8 bytes in.
    ProcessExifDir(ExifSection+8+FirstOffset, ExifSection+8, length-8, 0, exifInfo);

    exifInfo->ThumbnailAtEnd = exifInfo->ThumbnailOffset >= exifInfo->LargestExifOffset ? TRUE : FALSE;
    ...
    if (exifInfo->ptr.FocalplaneXRes != 0 && exifInfo->ptr.ExifImageWidth != 0){
        ...
        exifInfo->CCDWidth = (float)(exifInfo->ptr.ExifImageWidth * exifInfo->ptr.FocalplaneUnits / exifInfo->ptr.FocalplaneXRes);

        if (exifInfo->FocalLength && exifInfo->FocalLength35mmEquiv == 0){
            // Compute 35 mm equivalent focal length based on sensor geometry if we haven't
            // already got it explicitly from a tag.
            exifInfo->FocalLength35mmEquiv = (int)(exifInfo->FocalLength/exifInfo->CCDWidth*36 + 0.5);
        }
    }
}
...
void create_EXIF(ImageInfo_t* exifInfo)
{
    ...
    Put16u(Buffer+10, 0x2a, exifInfo->ptr.MotorolaOrder);

    DataWriteIndex = 16;
    Put32u(Buffer+12, DataWriteIndex-8, exifInfo->ptr.MotorolaOrder); // first IFD offset.  Means start 16 bytes in.

    {
        ...
        Put16u(Buffer+DirIndex, NumEntries, exifInfo->ptr.MotorolaOrder); // Number of entries
        DirIndex += 2;
  
        // Enitries go here...
        {
            // Date/time entry
            Put16u(Buffer+DirIndex, TAG_DATETIME, exifInfo->ptr.MotorolaOrder);         // Tag
            Put16u(Buffer+DirIndex + 2, FMT_STRING, exifInfo->ptr.MotorolaOrder);       // Format
            Put32u(Buffer+DirIndex + 4, 20, exifInfo->ptr.MotorolaOrder);               // Components
            Put32u(Buffer+DirIndex + 8, DataWriteIndex-8, exifInfo->ptr.MotorolaOrder); // Pointer or value.
            DirIndex += 12;

            DateIndex = DataWriteIndex;
            if (exifInfo->numDateTimeTags){
                // If we had a pre-existing exif header, use time from that.
                memcpy(Buffer+DataWriteIndex, exifInfo->DateTime, 19);
                Buffer[DataWriteIndex+19] = '\0';
            }else{
                // Oterwise, use the file's timestamp.
                FileTimeAsString(Buffer+DataWriteIndex, exifInfo);
            }
            DataWriteIndex += 20;
        
            // Link to exif dir entry
            Put16u(Buffer+DirIndex, TAG_EXIF_OFFSET, exifInfo->ptr.MotorolaOrder);      // Tag
            Put16u(Buffer+DirIndex + 2, FMT_ULONG, exifInfo->ptr.MotorolaOrder);        // Format
            Put32u(Buffer+DirIndex + 4, 1, exifInfo->ptr.MotorolaOrder);                // Components
            Put32u(Buffer+DirIndex + 8, DataWriteIndex-8, exifInfo->ptr.MotorolaOrder); // Pointer or value.
            DirIndex += 12;
        }
...

    {
        ...
        Put16u(Buffer+DirIndex, TAG_DATETIME_ORIGINAL, exifInfo->ptr.MotorolaOrder);         // Tag
        Put16u(Buffer+DirIndex + 2, FMT_STRING, exifInfo->ptr.MotorolaOrder);       // Format
        Put32u(Buffer+DirIndex + 4, 20, exifInfo->ptr.MotorolaOrder);               // Components
        Put32u(Buffer+DirIndex + 8, DataWriteIndex-8, exifInfo->ptr.MotorolaOrder); // Pointer or value.
        DirIndex += 12;
        ...
        // End of directory - contains optional link to continued directory.
        Put32u(Buffer+DirIndex, 0, exifInfo->ptr.MotorolaOrder);
    }

    {
        //Continuation which links to this directory;
        Put32u(Buffer+DirContinuation, DataWriteIndex-8, exifInfo->ptr.MotorolaOrder);
        DirIndex = DataWriteIndex;
        NumEntries = 2;
        DataWriteIndex += 2 + NumEntries*12 + 4;
        ...
        {
            // Link to exif dir entry
            Put16u(Buffer+DirIndex, TAG_THUMBNAIL_OFFSET, exifInfo->ptr.MotorolaOrder);         // Tag
            Put16u(Buffer+DirIndex + 2, FMT_ULONG, exifInfo->ptr.MotorolaOrder);       // Format
            Put32u(Buffer+DirIndex + 4, 1, exifInfo->ptr.MotorolaOrder);               // Components
            Put32u(Buffer+DirIndex + 8, DataWriteIndex-8, exifInfo->ptr.MotorolaOrder); // Pointer or value.
            DirIndex += 12;
        ...
        {
            // Link to exif dir entry
            Put16u(Buffer+DirIndex, TAG_THUMBNAIL_LENGTH, exifInfo->ptr.MotorolaOrder);         // Tag
            Put16u(Buffer+DirIndex + 2, FMT_ULONG, exifInfo->ptr.MotorolaOrder);       // Format
            Put32u(Buffer+DirIndex + 4, 1, exifInfo->ptr.MotorolaOrder);               // Components
            Put32u(Buffer+DirIndex + 8, 0, exifInfo->ptr.MotorolaOrder); // Pointer or value.
            DirIndex += 12;
        }
    ...
    RemoveSectionType(M_EXIF, exifInfo);
...
{
    int a;
    if (exifInfo->ptr.NumOrientations == 0) return NULL;

    for (a=0;a<exifInfo->ptr.NumOrientations;a++){
        switch(exifInfo->ptr.OrientationNumFormat[a]){
            case FMT_SBYTE:
            case FMT_BYTE:      
                *(uchar *)(exifInfo->ptr.OrientationPtr[a]) = 1;
                break;

            case FMT_USHORT:    
                Put16u(exifInfo->ptr.OrientationPtr[a], 1, exifInfo->ptr.MotorolaOrder);                
                break;

            case FMT_ULONG:     
            case FMT_SLONG:     
                memset(exifInfo->ptr.OrientationPtr[a], 0, 4);
                // Can't be bothered to write  generic Put32 if I only use it once.
                if (exifInfo->ptr.MotorolaOrder){
                    ((uchar *)exifInfo->ptr.OrientationPtr[a])[3] = 1;
                }else{
                    ((uchar *)exifInfo->ptr.OrientationPtr[a])[0] = 1;
                }
                break;
    ...
    if (exifInfo->Orientation >= 1 && exifInfo->Orientation <= 8){
        return OrientTab[exifInfo->Orientation];
    }else{
        return NULL;
    }
}
...
void ShowImageInfo(int ShowFileInfo, ImageInfo_t* exifInfo)
{
    if (ShowFileInfo){
        ...
        {
            char Temp[20];
            FileTimeAsString(Temp, exifInfo);
            printf("File date    : %s\n",Temp);
        }
    }

    if (exifInfo->CameraMake[0]){
        printf("Camera make  : %s\n",exifInfo->CameraMake);
        printf("Camera model : %s\n",exifInfo->CameraModel);
    }
    if (exifInfo->DateTime[0]){
        printf("Date/Time    : %s\n",exifInfo->DateTime);
    }
    ...
    if (exifInfo->Orientation > 1 && exifInfo->Orientation <=8){
        ...
        printf("Orientation  : %s\n", OrientTab[exifInfo->Orientation]);
    }

    if (exifInfo->IsColor == 0){
        printf("Color/bw     : Black and white\n");
    }

    if (exifInfo->FlashUsed >= 0){
        if (exifInfo->FlashUsed & 1){    
            printf("Flash used   : Yes");
            switch (exifInfo->FlashUsed){
    ...
    if (exifInfo->FocalLength){
        printf("Focal length : %4.1fmm",(double)exifInfo->FocalLength);
        if (exifInfo->FocalLength35mmEquiv){
            printf("  (35mm equivalent: %dmm)", exifInfo->FocalLength35mmEquiv);
        }
        printf("\n");
    }

    if (exifInfo->DigitalZoomRatio > 1){
        // Digital zoom used.  Shame on you!
        printf("Digital Zoom : %1.3fx\n", (double)exifInfo->DigitalZoomRatio);
    }

    if (exifInfo->CCDWidth){
        printf("CCD width    : %4.2fmm\n",(double)exifInfo->CCDWidth);
    }

    if (exifInfo->ExposureTime){
        if (exifInfo->ExposureTime < 0.010){
            printf("Exposure time: %6.4f s ",(double)exifInfo->ExposureTime);
        }else{
            printf("Exposure time: %5.3f s ",(double)exifInfo->ExposureTime);
        }
        if (exifInfo->ExposureTime <= 0.5){
            printf(" (1/%d)",(int)(0.5 + 1/exifInfo->ExposureTime));
        }
        printf("\n");
    }
    if (exifInfo->ApertureFNumber){
        printf("Aperture     : f/%3.1f\n",(double)exifInfo->ApertureFNumber);
    }
    if (exifInfo->Distance){
        if (exifInfo->Distance < 0){
            printf("Focus dist.  : Infinite\n");
        }else{
            printf("Focus dist.  : %4.2fm\n",(double)exifInfo->Distance);
        }
    }
    ...
    if (exifInfo->MeteringMode > 0){ // 05-jan-2001 vcs
        printf("Metering Mode: ");
        switch(exifInfo->MeteringMode) {
        ...
        default: printf("unknown (%d)\n",exifInfo->MeteringMode); break;
        }
    }

    if (exifInfo->ExposureProgram){ // 05-jan-2001 vcs
    ...
    if (exifInfo->DistanceRange) {
        printf("Focus range  : ");
        switch(exifInfo->DistanceRange) {
            case 1:
                printf("macro");
                break;
            case 2:
    ...
    if (exifInfo->Process != M_SOF0){
        ...
        for (a=0;;a++){
            ...
            if (ProcessTable[a].Tag == exifInfo->Process){
    ...
    if (exifInfo->GpsInfoPresent){
        printf("GPS Latitude : %s\n",exifInfo->GpsLat);
        printf("GPS Longitude: %s\n",exifInfo->GpsLong);
        if (exifInfo->GpsAlt[0]) printf("GPS Altitude : %s\n",exifInfo->GpsAlt);
    }

    if (exifInfo->QualityGuess){
        printf("JPEG Quality : %d\n", exifInfo->QualityGuess);
    }

    // Print the comment. Print 'Comment:' for each new line of comment.
    if (exifInfo->Comments[0]){
        int a,c;
        printf("Comment      : ");
        if (!exifInfo->CommentWidthchars){
            for (a=0;a<MAX_COMMENT_SIZE;a++){
                c = exifInfo->Comments[a];
                if (c == '\0') break;
                if (c == '\n'){
                    // Do not start a new line if the string ends with a carriage return.
                    if (exifInfo->Comments[a+1] != '\0'){
                        printf("\nComment      : ");
                    }else{
                        printf("\n");
                    }
            ...
            printf("%.*ls\n", exifInfo->CommentWidthchars, (wchar_t *)exifInfo->Comments);
        }
    }
}
...
{
    printf("\"%s\"",exifInfo->FileName);

    printf(" %dx%d",exifInfo->Width, exifInfo->Height);

    if (exifInfo->ExposureTime){
        if (exifInfo->ExposureTime <= 0.5){
            printf(" (1/%d)",(int)(0.5 + 1/exifInfo->ExposureTime));
        }else{
            printf(" (%1.1f)",exifInfo->ExposureTime);
        }
    }

    if (exifInfo->ApertureFNumber){
        printf(" f/%3.1f",(double)exifInfo->ApertureFNumber);
    }

    if (exifInfo->FocalLength35mmEquiv){
        printf(" f(35)=%dmm",exifInfo->FocalLength35mmEquiv);
    }

    if (exifInfo->FlashUsed >= 0 && exifInfo->FlashUsed & 1){
        printf(" (flash)");
    }
